...
import { ActivityIndicator , Picker , Alert , View, StyleSheet, TouchableOpacity , TextInput , Text, Image , AsyncStorage , Dimensions  , ScrollView } from 'react-native';
...
export default class BreedingGender extends React.Component {
  constructor(props) {
    super(props);
        ...
        this.uploadImage(this.state.localUri,this.guidGenerator()+"_"+currentDate).then(async(imageUrl) => {
          ...
          await axios.post(url,data,{
              ...
              if(res.data['data']['success']=="true"){
                this.setState({gender:res.data['data']['gender'],accuracy:res.data['data']['accuracy'].toString(),nextBtn:true})
              }
          })
        })
        .catch((error) => {
  ...
  next=()=>{
    this.props.navigation.navigate("BreedingSuitability",{ type:this.state.type.toLowerCase(),gender:this.state.gender.toLowerCase()})
  }
  ...
  render() {
    ...
    return (
      <ScrollView style={styles.scrollView}>
        ...
        <View style={styles.center}>
          {this.state.nextBtn ? (
            <TouchableOpacity style={[styles.buttonContainer, styles.loginButton]} onPress={this.next}>
              <Text style={{color: '#000000', fontWeight: 'bold'}}>Next</Text>
            </TouchableOpacity>
          ) : null}
        </View>
