...
import {FISH_GROWTH_URL} from '@env';
...
const FishGrowthIdentification = () => {
  ...
  const getPredication = async params => {
      return new Promise((resolve, reject) => {
          ...
          const url = {FISH_GROWTH_URL};
          //console.log(bodyFormData)
          return axios
            .post(url.FISH_GROWTH_URL, bodyFormData, {
              headers: {
  ...
  const clearOutput = () => {
    ...
    return (
      <View style={[backgroundStyle, styles.outer] }>
        ...
        <ImageBackground
          blurRadius={7}
          source={require('../../../assets/fishdocback.jpg')}
          style={{height: height, width: height}}
        />
         <Text style={styles.title}>{'Fish Growth Prediction'}</Text>
        <TouchableOpacity 
        onPress={clearOutput} 
...

        {
          (result && label && (
            <View style={styles.mainOuter}>
              ...
              <TouchableOpacity
                activeOpacity={0.9}
                onPress={() => navigation.navigate('FishGrowthGuide', { month: label})}
                 style={styles.disAgreebtnStyle}
              >
                <Text>Not Agree</Text>
              </TouchableOpacity>
          ...
          (image && <Text style={styles.emptyText}>{label}</Text>) || (
            <Text style={styles.emptyText}>
              Use below buttons to select a picture of a your fish .
            </Text>
          )
        }
...
export default FishGrowthIdentification;